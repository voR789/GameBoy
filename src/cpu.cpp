...
// Helper functions
uint8_t  cpu::getUpper(uint16_t data) { return ((data & 0xFF00) >> 8); }
uint8_t  cpu::getLower(uint16_t data) { return (data & 0xFF); }
void     cpu::fetchOpcode() { opcode = MMU.readMem(pc++); }
uint8_t  cpu::fetchNextByte() { return MMU.readMem(pc++); }
...
uint8_t cpu::popStack() {
    ...
    // the stack in the gb memory map is reverse mapped, starting at 0x..., and decreasing to 0x..., so ++ is "popping"
    // in this context
}
...
void cpu::cpA(uint8_t byte) {  // basically subA, but just to set flags
    uint8_t A = getUpper(registers[0]);
        ...
        case (0x2): {
            switch (secondNibble) {
                ...
                case (0x7): {
                    ...
                    if (getFlag('N')) {  // bcd subtraction logic -> only based upon flags
                        if (getFlag('C')) {
                            A -= 0x60;
                        }
                        if (getFlag('H')) {
                    ...
                    } else {  // bcd addition logic -> addition can cause overflows with no flags
                        if (getFlag('C') || A > 0x99) {
                            A += 0x60;
                            setFlag('C');
                        }
...
int cpu::step() {
    ...
    if (halt) {
        if (MMU.readMem(0xFFFF) & MMU.readMem(0xFF0F)) {
            ...
            } else { haltBugFlag = true; }
        } 
        else {
            // Still handle EI for edge case
            if (EI_COUNTER > 0) {
                EI_COUNTER--;
                if (EI_COUNTER == 0) { IME = true; }
            }
